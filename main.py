...
Modified by: ktiays <cuansky@gmail.com>
...
class UserInterface:
    ...
    def init(self):
        if not os.path.exists(self.configFile):
            ...
            try:
                ...
                print("配置文件解析失败，请检查配置文件是否正确。错误为：")
                print(e)
    ...
    def changeTime(self):
        ...
        try:
            index = input(
                "请输入要删除的预约序号（多个用英文逗号隔开，如1,2,3，输入0表示修改所有方案）："
            )
            ...
            print(
                "请注意，**本模块不对预约时间和预约时长的合法性进行检查**，请您自行检查，错误的时间可能导致**封号一周**的惩罚。"
            )
            print(
                "请输入开始使用时间（格式为yyyy-mm-dd hh:mm:ss，如2023-01-01 12:00:00）："
            )
            time = input()
            time = datetime.strptime(time, DATETIME_FORMAT)
            hours = int(input("请输入使用时长，单位为小时："))
            if hours < 0:
    ...
    def startAt(self):
        try:
            startTime = input(
                "请输入程序开始运行时间（格式为yyyy-mm-dd hh:mm:ss，如2023-01-01 12:00:00）："
            )
            startTime = datetime.strptime(startTime, DATETIME_FORMAT)
            self._start_at(startTime)
        except KeyboardInterrupt:
            print("程序终止")
            return
        except Exception as e:
    ...
    def _start_at(
        self,
        start_time: datetime,
        begin_time_mapper: Callable[[datetime], datetime] = None,
    ):
        ...
        while True:
            ...
            sleep(1)
        self._start(begin_time_mapper)

    def _start(self, begin_time_mapper: Callable[[datetime], datetime] = None):
        for retryCnt in range(self.killer.cfg["settings"]["max_try_times"]):
            ...
            for i, plan in enumerate(self.killer.plans):
                ...
                if res["CODE"] == "ok":
                    ...
                    print(
                        f"\r第{i+1}个方案预约失败，原因为："
                        + "\033[0;31m%s\033[0m" % res["MESSAGE"]
                    )
    ...
    def start_polling(self):
        try:
            while True:
                ...
                def next_begin_time(begin_time: datetime) -> str:
                    now = datetime.now()
                    delta = 1 if now.hour < 20 else 2
                    target = datetime.combine(
                        now + timedelta(days=delta), begin_time.time()
                    )
                    return target

                # Find the next 20:00
                now = datetime.now()
                next_eight = now.replace(hour=20, minute=0, second=0, microsecond=0)
                if now > next_eight:
                    next_eight += timedelta(days=1)

                self._start_at(
                    start_time=next_eight,
                    begin_time_mapper=next_begin_time,
                )
        except KeyboardInterrupt:
            return
    ...
    def addPlan(self):
        try:
            ...
            if self.th.is_alive():
                print(
                    "正在初始化楼层和座位信息（为避免频繁请求而导致封号，此过程可能需要几秒，请耐心等待）"
                )
                for _ in "loading...":
                    print(_, end="", flush=True)
                    sleep(0.5 if self.th.is_alive() else 0.1)
            ...
            print(
                f'本房间最早开放时间为：{room["range"]["minBeginTime"]}时，最晚开放时间为：{room["range"]["maxEndTime"]}时'
            )
            print(
                "请输入开始使用时间（格式为yyyy-mm-dd hh:mm:ss，如2023-01-01 12:00:00）："
            )
            ...
            time = datetime.strptime(time, DATETIME_FORMAT)
            if (
                time.hour < room["range"]["minBeginTime"]
                or time.hour > room["range"]["maxEndTime"]
            ):
            ...
            for seat in seats:
                ...
                if len(seatInfo) > 1:
                    raise Exception(
                        f"程序错误，{floorName}中座位{seat}存在多个\n" + str(seatInfo)
                    )
    ...
    def setSettings(self):
        try:
            ...
            sleep(0.1)
            time = input(
                "请输入重试间隔（单位为秒），过小的重试间隔有可能导致**封号一周**的处罚，强烈建议该值不小于5秒："
            )
