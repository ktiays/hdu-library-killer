...
class Killer:
    ...
    def __queryRooms(self):
        ...
        for room in rooms.keys():
            rooms[room] = self.session.get(
                url=self.urls["query_seats"] + "?" + rooms[room]
            ).json()["data"]
            sleep(2)
        return rooms
    ...
    def __querySeats(self):
        ...
        for room in self.rooms.keys():
            data = {
                ...
                "duration": 3600,
                "num": 1,
                "space_category[category_id]": self.rooms[room]["space_category"][
                    "category_id"
                ],
                "space_category[content_id]": self.rooms[room]["space_category"][
                    "content_id"
                ],
            }
            resp = self.session.post(url=self.urls["query_seats"], data=data).json()
            self.rooms[room]["floors"] = {
                x["roomName"]: x
                for x in resp["allContent"]["children"][2]["children"]["children"]
            }
            for floor in self.rooms[room]["floors"].keys():
                self.rooms[room]["floors"][floor]["seats"] = self.rooms[room]["floors"][
                    floor
                ]["seatMap"]["POIs"]
            sleep(2)
    ...
    def plan2data(
        self, plan, begin_time_mapping: Callable[[dt.datetime], dt.datetime] = None
    ):
        data = {}
        begin_time: dt.datetime = plan["beginTime"]
        if begin_time_mapping is not None:
            begin_time = begin_time_mapping(begin_time)
